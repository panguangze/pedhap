...
from whatshap.read_set import ReadSet
...
class VariantTable:
    ...
    def phase_single(self,reads_set: List[Read], variant: VcfVariant):
        for r in reads_set:
            if r[0][0] <= variant.position and r[-1][0] >= variant.position:
                return r.find_allele(variant.position)
        return -1
    def extend_by_readset(self, sample1: str,reads_set: ReadSet):
        try:
            sample1_index = self._sample_to_index[sample1]
        except KeyError:
            return
        
        sample1_phase = self.phases[sample1_index]
        hap_info = {}
        for p in sample1_phase:
            phased_block_id = -1
            need_reverse = False
            if p.block_id in hap_info.keys():
                phased_block_id = hap_info[p.block_id][0]
                need_reverse = hap_info[p.block_id][1]
            else:
                phased_block_id, need_reverse = reads_set.get_phase_id(p.block_id)
            if phased_block_id != -1:
                p.block_id = phased_block_id
                if need_reverse:
                    t = p.phase[0]
                    p.phase[0] = p.phase[1]
                    p.phase[1] = t
        ...
        for variant, genotype, phase in zip(
            self.variants, self.genotypes[sample1_index], self.phases[sample1_index]
        ):  
    ...
    def phased_blocks_as_reads(
        ...
        try:
            ...
            sample1_genotypes = self.genotypes[sample1_index]
        except KeyError:
            return
        ...
        assert (
            len(self.variants)
            == len(self.genotypes[sample2_index])
            == len(self.phases[sample2_index])
        )
        for i, (variant, genotype, phase) in enumerate(zip(
            self.variants, self.genotypes[sample2_index], self.phases[sample2_index]
        )):  
            # target homo skip,
            if sample1_genotypes[i].is_homozygous:
                continue
            if len(genotype.as_vector()) > 2:
                # only use diploid variants
                continue
            ...
            if phase.block_id in read_map:
                ...
                r = Read(
                    "{}_block_{}".format(sample1, phase.block_id), mapq, source_id, numeric_sample_id
                )
                r.add_variant(variant.position, phase.phase[0], quality)
                read_map[phase.block_id] = r
            if sample1_phases[i] != None:
                side = 0
                if phase.phase[0] == sample1_phases[i].phase[1]:
                    side = 1
                read_map[phase.block_id].set_covered_block(sample1_phases[i].block_id, side)
            else:
                p = VariantCallPhase(block_id=phase.block_id, phase=phase, quality=quality)
                sample1_phases[i] = p
